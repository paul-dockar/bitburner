...
export async function main(ns) {
    ...
    let actualScriptTime = ns.formulas.hacking.weakenTime(server, player);
    ...
    ns.tprintf(row, "WEAK", startTime.toISOString(), finishTime.toISOString(), lag, Math.round(waitTime, 3), "", Math.round(scriptTime, 2), Math.round(actualScriptTime, 2), maxScriptTime, ns.args[4]);
}