...
export async function main(ns) {
    ...
    let actualScriptTime = ns.formulas.hacking.hackTime(server, player);
    ...
    ns.tprintf(row, "HACK", startTime.toISOString(), finishTime.toISOString(), lag, Math.round(waitTime, 3), money, Math.round(scriptTime, 2), Math.round(actualScriptTime, 2), maxScriptTime, ns.args[4]);
}